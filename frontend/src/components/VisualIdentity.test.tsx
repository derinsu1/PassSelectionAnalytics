...
describe("VisualIdentity", () => {
  it("uses downloaded local assets and keeps unresolved records blank", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: true, json: async () => ({ clubs: [{ source_id: 7, status: "placeholder", local_path: null }], players: [{ source_id: 99, status: "downloaded", local_path: "/data/visual-assets/players/99.png" }] }) }));
    ...
    await waitFor(() => expect(portrait).toHaveAttribute("src", "/data/visual-assets/players/99.png"));
