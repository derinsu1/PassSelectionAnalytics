...
const PLAYER_METRIC_HELP: Record<string, string> = {
  ...
  pvi: "Project-owned 55% Local xPass and 45% bounded actual-frame delta-xT utility score. Availability is a visible local lane-risk diagnostic, not a PVI input. It is not a player rating.",
...
function comparisonDelta(anchor: PlayerPercentile | null, comparison: PlayerPercentile | null) {
  if (!anchor || !comparison || anchor.value === null || comparison.value === null) return "N/A";
...
function ComparisonPercentile({ metric }: { metric: PlayerPercentile | null }) {
  if (!metric) return <span className="percentile-badge percentile-badge--unavailable">N/A</span>;
