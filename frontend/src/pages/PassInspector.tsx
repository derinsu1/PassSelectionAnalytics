...
export function PassComparison({ selected, inspected, playerHref }: { selected: ReceiverOption | null; inspected: ReceiverOption | null; playerHref: (playerId: number) => string }) {
  ...
  const metrics: Array<{ label: string; help: string; selected: string; inspected: string }> = [
    { label: "Open xT", help: metricHelp.sameFrameOpenXt, selected: formatNumber(selected.same_frame.open_xt, 5), inspected: formatNumber(inspected.same_frame.open_xt, 5) },
    ...
    { label: "Open xT rank", help: metricHelp.sameFrameRank, selected: selected.same_frame.rank?.toString() ?? "—", inspected: inspected.same_frame.rank?.toString() ?? "—" },
    ...
    { label: "Local xPass rank", help: metricHelp.localXPassRank, selected: selected.local_xpass.rank?.toString() ?? "—", inspected: inspected.local_xpass.rank?.toString() ?? "—" },
    ...
    { label: "PVI rank", help: metricHelp.pviRank, selected: selected.pass_viability.rank?.toString() ?? "—", inspected: inspected.pass_viability.rank?.toString() ?? "—" },
  ...
  const issues = [selected, ...(isSameOption ? [] : [inspected])].flatMap((option) => {
    ...
    if (option.same_frame.coordinate_in_playing_area === false) messages.push(`${prefix}: ${formatNumber(option.same_frame.out_of_bounds_distance_m, 2)} m outside the pitch; Open xT uses the nearest boundary.`);
